...
export default function Page() {
  ...
  /**
   * Get an access token for audience 'test-3'.
   * @param e - React mouse event
   */
  async function getTokenForAudience3(
    e: ReactMouseEvent<HTMLButtonElement, MouseEvent>
  ) {
    "use server";

    const token = await auth0.getAccessToken({ audience: "{YOUR_THIRD_API_IDENTIFIER_HERE}" });
          ...
          <div className="grid auto-rows-min gap-4 md:grid-cols-3">
            ...
            <div className="aspect-video rounded-xl bg-muted/50 p-4">
              ...
              <button
                type="button"
                className="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2"
                onClick={getTokenForAudience3}
              >
                Get Token (Audience 3)
              </button>
            </div>
            <div className="aspect-video rounded-xl bg-muted/50 p-4">
              <p className="mb-3 text-lg text-gray-500 md:text-xl dark:text-gray-400">
