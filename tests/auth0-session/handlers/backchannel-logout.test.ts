...
describe('backchannel-logout', () => {
  ...
  it('should fail when logout_token is missing', async () => {
    const baseURL = await setup({ ...defaultConfig, session: { store: new Store(), genId: () => 'foo' } });
  ...
  it('should succeed with valid logout token', async () => {
    ...
    const baseURL = await setup({ ...defaultConfig, session: { store, genId: () => 'foo' } });
    const { res } = await post(baseURL, '/backchannel-logout', {
      body: { logout_token: await makeLogoutToken({ sid: 'foo' }) },
      fullResponse: true
    });
    await expect(store.get('https://op.example.com/|foo')).resolves.toMatchObject({
      header: {
  ...
  it('should save sid and sub', async () => {
    ...
    await expect(store.get('https://op.example.com/|bar')).resolves.toMatchObject({
      data: {}
    });
    expect(res.statusCode).toEqual(204);
  });
