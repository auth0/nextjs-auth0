...
import { getResponse, GetResponseOpts, LoginOpts, login as appRouterLogin } from '../fixtures/app-router-helpers';
...
const getAccessTokenResponse = async ({
  ...
  if (authenticated) {
    ...
    if (userRsc) {
      mocked(nextCookies).mockImplementation(() => loginRes.cookies);
    }
  }
  ...
  return getResponse({
    auth0Instance,
    url: userRsc ? '/api/auth/access-token-rsc' : '/api/auth/access-token',
    extraHandlers: {
...
describe('get access token', () => {
  ...
  describe('app router (server component)', () => {
    test('should return an access token', async () => {
      const res = await getAccessTokenResponse({
        authenticated: true,
        userRsc: true
