...
// ============================================================================
// deleteAuthenticator Scenarios
// ============================================================================

export const deleteAuthenticatorScenarios: MfaTestScenario<
  {
    mfaToken: string;
    authenticatorId: string;
  },
...
>[] = [
  {
    name: "delete authenticator success",
    input: {
      mfaToken: "encrypted-token",
      authenticatorId: "auth_123"
    },
  ...
  {
    name: "Auth0 404 error (not found)",
    input: {
      mfaToken: "encrypted-token",
      authenticatorId: "auth_nonexistent"
    },
    mswResponse: {
      status: 404,
      body: {
    ...
    expectError: (error: any) => {
      if (error.code !== "not_found") throw new Error("Expected not_found");
    }
  },
  {
    name: "Auth0 500 error",
    ...
    expectError: (error: any) => {
      if (!error.message.includes("delete"))
        throw new Error("Expected error about deletion");
    }
  }
];
