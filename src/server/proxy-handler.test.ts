...
const DEFAULT = {
  ...
  upstreamBaseUrl: "https://api.internal.example.com",
...
describe("Authentication Client - Custom Proxy Handler", async () => {
  beforeEach(async () => {
    authClient = new AuthClient({
      ...
      proxyRoutes: [
        {
          proxyPath: DEFAULT.proxyPath,
          targetBaseUrl: DEFAULT.upstreamBaseUrl,
    ...
    it("10.3 should handle concurrent requests to different proxy routes independently", async () => {
      ...
      const multiProxyClient = new AuthClient({
        ...
        proxyRoutes: [
          {
            proxyPath: "/api-1",
            targetBaseUrl: "https://api1.example.com",
      ...
      server.use(
        http.get("https://api1.example.com/data", () => {
          api1CallCount++;
          return HttpResponse.json({ api: "api1" });
        }),
        http.get("https://api2.example.com/data", () => {
          api2CallCount++;
          return HttpResponse.json({ api: "api2" });
        })
      );

      // Make concurrent requests to different proxies
      const requests = [
        multiProxyClient.handler(
          new NextRequest(new URL("/api-1/data", DEFAULT.appBaseUrl), {
            method: "GET",
            headers: { cookie }
          })
        ),
        multiProxyClient.handler(
          new NextRequest(new URL("/api-2/data", DEFAULT.appBaseUrl), {
            method: "GET",
            headers: { cookie }
          })
        ),
      ...
      // Both APIs should receive correct number of calls
      expect(api1CallCount).toBe(2);
      expect(api2CallCount).toBe(1);
    });
  });
