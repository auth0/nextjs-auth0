...
export const addOrUpdateFederatedTokenToSession = (
  ...
  if (!session.federatedConnectionsMap) {
    session.federatedConnectionsMap = {}
  }

  session.federatedConnectionsMap[fcat.connection] = serializedFCTokenSet
  return session
}
...
export const serializeFederatedTokens = async (
  ...
  options: EncryptAndSetCookieOptions
): Promise<void> => {
...
export const findFederatedToken = (
  ...
  const partialTokenSet = session.federatedConnectionsMap?.[provider]
