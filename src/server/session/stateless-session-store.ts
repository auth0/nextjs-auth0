...
import { EncryptAndSetCookieOptions } from "../cookies"
import {
...
export class StatelessSessionStore extends AbstractSessionStore {
  ...
  async get(reqCookies: cookies.RequestCookies): Promise<SessionData | null> {
    const session = await cookies.decryptAndGet<SessionData>({
      reqCookies,
      cookieName: this.sessionCookieName,
      secret: this.secret,
    })
    if (session) {
      session.federatedConnectionsMap = (await deserializeFederatedTokens(reqCookies, this.secret)) ?? {}
    }
    return session
  }
  ...
  async set(
    ...
    const { federatedConnectionsMap: fcMap, ...originalSession } = session

    const maxAge = this.calculateMaxAge(originalSession.internal.createdAt)

    const setCookieOptions: EncryptAndSetCookieOptions = {
      reqCookies,
      resCookies,
    ...
    await cookies.encryptAndSet(setCookieOptions)
    if(fcMap){
      await serializeFederatedTokens(fcMap, setCookieOptions)
    }
  }
