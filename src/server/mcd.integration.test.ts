...
describe("MCD Integration Tests (Units 6-12)", () => {
  ...
  describe("Pre-MCD Session Backfill with Iss-Inference (Integration)", () => {
    ...
    it("I8: normalizeDomain correctly extracts hostname from iss URL", async () => {
      ...
      // normalizeDomain should extract hostname from the issuer URL
      expect(normalized.domain).toBe("custom.domain.auth0.com");
    ...
    it("I9: Backfill works with bare domain (no https:// prefix) in iss", async () => {
      ...
      // normalizeDomain should handle bare domains and construct HTTPS issuer
