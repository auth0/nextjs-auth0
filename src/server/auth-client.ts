...
export interface Routes {
  ...
  mfaVerify: string;
  mfaEnroll: string;
  mfaAssociate: string; // alias for mfaEnroll, matches Auth0 API path
}
...
export class AuthClient {
   ...
   *
   * Body: {mfa_token, challenge_type, authenticator_id?} (accepts camelCase for backward compat)
   *
   * Response: 200 + ChallengeApiResponse (snake_case)
  ...
  async handleChallenge(req: NextRequest): Promise<NextResponse> {
    try {
      ...
      // mfa_token in body (matches Auth0 API contract for /mfa/challenge)
      // snake_case preferred; camelCase fallback (deprecated)
      const mfaToken = validateStringFieldAndThrow(
        bodyRecord.mfa_token ?? bodyRecord.mfaToken,
        "mfa_token"
      );
      const challengeType = validateStringFieldAndThrow(
        bodyRecord.challenge_type ?? bodyRecord.challengeType,
        "challenge_type"
      );
      const rawAuthenticatorId =
        bodyRecord.authenticator_id ?? bodyRecord.authenticatorId;
      const authenticatorId = rawAuthenticatorId
        ? validateStringFieldAndThrow(rawAuthenticatorId, "authenticator_id")
        : undefined;
   ...
   * Headers:
   *   Authorization: Bearer <encrypted-mfa-token>
   * Body: {authenticator_types, oob_channels?, phone_number?, email?} (accepts camelCase for backward compat)
   *
   * Response: 200 + EnrollmentApiResponse (snake_case)
   * Error: 400 + {error, error_description}
   */
  async handleEnroll(req: NextRequest): Promise<NextResponse> {
    try {
      ...
      const authenticatorTypes = validateArrayFieldAndThrow(
        bodyRecord.authenticator_types ?? bodyRecord.authenticatorTypes,
        "authenticator_types"
      );
   ...
   * Headers:
   *   Authorization: Bearer <encrypted-mfa-token>
   * Body: {otp | oob_code+binding_code | recovery_code} (accepts camelCase for backward compat)
   *
   * Response: 200 + MfaVerifyResponse + Set-Cookie
   ...
   * Returns raw Auth0 API response (snake_case).
   *
   ...
   * @returns Enrollment response (snake_case) with authenticator details and optional recovery codes
   * @throws {MfaTokenInvalidError} If token cannot be decrypted
   * @throws {MfaTokenExpiredError} If token expired
