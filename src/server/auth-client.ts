...
export class AuthClient {
  ...
  async handler(req: NextRequest): Promise<NextResponse> {
    ...
    if (method === "GET" && sanitizedPathname === this.routes.login) {
      ...
      return this.#unwrap(() => this.handleLogout(auth0Req, auth0Res));
    } else if (method === "GET" && sanitizedPathname === this.routes.callback) {
      return this.#unwrap(() => this.handleCallback(auth0Req, auth0Res));
    } else if (method === "GET" && sanitizedPathname === this.routes.profile) {
      return this.#unwrap(() => this.handleProfile(auth0Req, auth0Res));
    } else if (
      method === "GET" &&
      sanitizedPathname === this.routes.accessToken &&
      this.enableAccessTokenEndpoint
    ) {
      return this.#unwrap(() => this.handleAccessToken(auth0Req, auth0Res));
    } else if (
      method === "POST" &&
      ...
      return this.#unwrap(() => this.handleConnectAccount(auth0Req, auth0Res));
    } else if (sanitizedPathname.startsWith("/me/")) {

      // TODO: This will never work when using Api Routes, as it will always start with `/api/auth/` (or wherever it is getting mounted).
      // We should solve this.
      return this.#unwrap(() => this.handleMyAccount(auth0Req, auth0Res));
