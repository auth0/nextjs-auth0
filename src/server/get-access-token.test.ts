...
describe("Auth0Client - getAccessToken", () => {
  ...
  it("should refresh token early when expiresAt is within tokenRefreshLeeway", async () => {
    vi.useFakeTimers();
  ...
  it("should not refresh when expiresAt is outside tokenRefreshLeeway", async () => {
    vi.useFakeTimers();
    const now = new Date("2026-01-01T00:00:00.000Z");
    vi.setSystemTime(now);
