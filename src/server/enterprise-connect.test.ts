...
describe("applyEnterpriseConnectRestrictions", () => {
  describe("methods", () => {
    it("leaves allowed methods callable", () => {
      class FakeClient {
        middleware() { return "ok"; }
        startInteractiveLogin() { return "ok"; }
        startEnterpriseLogin() { return "ok"; }
        customTokenExchange() { return "ok"; }
      }
      ...
      for (const method of EC_ALLOWED_METHODS) {
        ...
        expect(() => (instance as unknown as Record<string, () => unknown>)[method]()).not.toThrow();
      }
    });
    ...
    it("blocks methods not in the allowlist", async () => {
      ...
      expect((instance as unknown as Record<string, () => unknown>).allowed()).toBe("ok");
      await expect(
        (instance as unknown as Record<string, () => Promise<unknown>>).blocked()
    ...
    it("includes the method name in the blocked error message", async () => {
      ...
      await expect(
        (instance as unknown as Record<string, () => Promise<unknown>>).doSomething()
      ).rejects.toThrow(/doSomething\(\) is not available when enterpriseConnect is true/);
    ...
    it("async blocked methods return a rejected promise, not a synchronous throw", () => {
      ...
      let result: unknown;
      expect(() => {
        result = (instance as unknown as Record<string, () => unknown>).asyncBlocked();
      }).not.toThrow();
    ...
    it("sync methods listed in EC_SYNC_METHODS throw synchronously", () => {
      ...
      expect(() => (instance as unknown as Record<string, () => unknown>).syncMethod())
  ...
  describe("getters", () => {
    it("leaves allowed getters accessible", () => {
      ...
      expect((instance as unknown as Record<string, unknown>).allowedGetter).toBe("ok");
    ...
    it("blocks getters not in the allowlist", () => {
      ...
      expect(() => (instance as unknown as Record<string, unknown>).blockedGetter)
        .toThrow(InvalidConfigurationError);
    });

    it("includes the getter name in the blocked error message", () => {
      class FakeClient {
        get myGetter() { return "x"; }
      }
      ...
      expect(() => (instance as unknown as Record<string, unknown>).myGetter)
  ...
  describe("guidance messages", () => {
    it("appends guidance when the member has an entry in EC_MEMBER_GUIDANCE", () => {
      ...
      return expect(
        (instance as unknown as Record<string, () => Promise<unknown>>).getSession()
      ).rejects.toThrow(/Read the user from the session store you populated in onCallback/);
    });

    it("falls back to the generic message when no guidance is registered", () => {
      class FakeClient {
        unknownMethod() { return "x"; }
      }
      ...
      return expect(
        (instance as unknown as Record<string, () => Promise<unknown>>).unknownMethod()
      ).rejects.toThrow(/unknownMethod\(\) is not available when enterpriseConnect is true\./);
    });
  });
});
