  ...
  describe("handleCallback", async () => {
    ...
    describe("beforeSessionSaved hook", async () => {
      ...
      it("should call beforeSessionSaved with updated tokens after token refresh (handleAccessToken)", async () => {
        ...
        // Track the HTTP request to the token endpoint
        let capturedTokenRequest: Request | undefined;
        ...
        const authClient = new AuthClient({
          ...
          fetch: getMockAuthorizationServer({
            ...
            // Capture the HTTP request to verify the HTTP layer contract
            onTokenRequest: async (request: Request) => {
              capturedTokenRequest = request.clone();
            }
          }),
          ...
          beforeSessionSaved: async (session) => {
            ...
            // Hook can modify the session (e.g., enrich with data from resource server using the access token)
            return {
              ...session,
              user: {
                ...session.user,
        ...
        // Verify the HTTP request to the token endpoint contains the correct parameters
        expect(capturedTokenRequest).toBeDefined();
        const requestBody = await capturedTokenRequest!.text();
        const params = new URLSearchParams(requestBody);

        // Verify grant type is refresh_token (not authorization_code)
        expect(params.get("grant_type")).toEqual("refresh_token");

        // Verify the refresh token from the session is sent to the token endpoint
        expect(params.get("refresh_token")).toEqual(DEFAULT.refreshToken);

        // Verify client authentication is included
        expect(params.get("client_id")).toEqual(DEFAULT.clientId);
        ...
        // === SESSION HOOK LAYER VALIDATION ===
        // CRITICAL ASSERTIONS - This is the bug being fixed:
        // The hook should have received the UPDATED access token (not the old one)
        // This was the bug: beforeSessionSaved was being called with the original session,
        // not the merged session with the updated tokens from sessionChanges.
        ...
        expect(hookReceivedSession?.tokenSet?.accessToken).toEqual(
          newAccessToken
        );
        expect(hookReceivedSession?.tokenSet?.refreshToken).toEqual(
          DEFAULT.refreshToken
        );

        // === SESSION PERSISTENCE LAYER VALIDATION ===
        ...
        // Updated token should be in final session (full integration validation)
