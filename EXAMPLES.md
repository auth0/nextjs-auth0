...
> [!TIP]  
> To refresh access tokens slightly before they expire across all `getAccessToken()` calls, set `tokenRefreshLeeway` on the `Auth0Client`:
> ```typescript
> export const auth0 = new Auth0Client({
>   tokenRefreshLeeway: 60 // refresh tokens up to 60s before expiry
>   // ... other config
> });
> ```
